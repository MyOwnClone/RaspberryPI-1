...
#import "bcm_host.h"
#import "cube_texture_and_coords.h"
...
/**
 * Method used to set up the drawing surface.
 */
- (void)initOGLWithState;

/**
 * Init the model projection.
 */
- (void)initModelProjection;
...
- (void)start
{
    ...
    // Start OGLES
    [self initOGLWithState];

    // Setup the model world
    [self initModelProjection];
...
- (void)loadTextureImages
{
    ...
    BOOL isDir = NO;
    if ([fm fileExistsAtPath:PATH_STRING isDirectory:&isDir] && isDir)
    { 
       NSLog(@"SubPaths: %@", [fm subpathsAtPath:PATH_STRING]);
    }
